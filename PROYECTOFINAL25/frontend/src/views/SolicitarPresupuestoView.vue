            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div class="md:col-span-2">
                <label class="flex flex-col min-w-40 flex-1">
                  <p class="text-gray-200 text-sm font-medium leading-normal pb-2">Servicio Seleccionado</p>
                  <input 
                    class="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-lg text-white border border-white/20 bg-[#2C2C2C] cursor-not-allowed h-12 placeholder:text-gray-400 p-3 text-base font-normal leading-normal focus:ring-0 focus:outline-none focus:border-white/20" 
                    readonly 
                    type="text" 
                    :value="servicioInfo.nombre"
                  />
                </label>
              </div>
              <div>
                <label class="flex flex-col min-w-40 flex-1">
                  <p class="text-gray-200 text-sm font-medium leading-normal pb-2">Precio del Servicio (Referencia)</p>
                  <input 
                    class="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-lg text-white border border-white/20 bg-[#2C2C2C] cursor-not-allowed h-12 placeholder:text-gray-400 p-3 text-base font-normal leading-normal focus:ring-0 focus:outline-none focus:border-white/20" 
                    readonly 
                    type="text" 
...
onMounted(async () => {
  // Cargar información del artista y servicio
  ...
  try {
    ...
    const resServicio = await fetch(`http://localhost:3000/api/artistas/${artistaId}/servicios/${servicioId}`)
    const dataServicio = await resServicio.json()
    servicioInfo.value = dataServicio
...
async function enviarSolicitud() {
  ...
  try {
    const response = await fetch('http://localhost:3000/api/contratos/solicitar', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        servicio_id: route.params.servicioId,
